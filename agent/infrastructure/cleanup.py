...
def compute_cleanup_base_time(debug_folder: Path, keep_count: int = DEFAULT_KEEP_LOG_COUNT) -> datetime:
...
def cleanup_maafw_bak_logs(debug_folder: Path, keep_count: int = DEFAULT_KEEP_LOG_COUNT) -> datetime:
    """清理旧的 maafw.bak.*.log 文件，保留最新的 keep_count 个。"""
    log_files = list(debug_folder.glob("maafw.bak.*.log"))
    logger.info(f"[日志清理] 找到日志总数: {len(log_files)}")
    if not log_files:
        logger.info("[日志清理] 无符合格式的日志，跳过")
        return DEFAULT_BASE_TIME
    ...
    for log_path in log_files:
        ...
        if dt:
            ...
            logger.warning(f"[日志清理] 无法解析日志时间,跳过: {log_path.name}")

    if not logs_with_time:
        logger.info("[日志清理] 所有日志均无法解析时间,跳过")
        return DEFAULT_BASE_TIME
    ...
    if not to_delete_logs:
        logger.info("[日志清理] 没有需要删除的日志")
        return DEFAULT_BASE_TIME

    logger.info(f"[日志清理] 待删除旧日志: {len(to_delete_logs)} 个")
    for dt, log_path in to_delete_logs:
        try:
            log_path.unlink()
            logger.info(f"[日志清理] 已删除: {log_path.name} (时间 {dt})")
        except Exception as e:
            logger.error(f"[日志清理] 删除失败 {log_path.name}: {e}")

    deleted_latest = to_delete_logs[0][0]
    logger.info(f"[日志清理] 被删除日志中最晚时间: {deleted_latest}")
    return deleted_latest


def clean_images_in_dir(debug_folder: Path, sub_dir: str, base_time: datetime) -> CleanupResult:
    """清理指定子目录下图片时间早于 base_time 的文件。"""
    result = CleanupResult()
    target_dir = debug_folder / sub_dir
    if not target_dir.exists():
        logger.info(f"[图片清理] 目录不存在,跳过: {target_dir}")
        return result
    ...
    if result.scanned == 0:
        logger.info(f"[图片清理] {sub_dir} 目录下无图片文件,跳过")
        return result
    ...
    for img_path in img_files:
        ...
        if img_dt is None:
            result.skipped += 1
            logger.warning(f"[图片清理] 无法解析图片时间,跳过: {img_path.name}")
            continue
        if img_dt < base_time:
    ...
    for img_dt, img_path in to_delete:
        try:
            img_path.unlink()
            result.deleted += 1
            logger.info(f"[图片清理] 删除图片: {img_path.name} (时间 {img_dt})")
        except Exception as e:
            result.errors += 1
            logger.error(f"[图片清理] 删除失败 {img_path.name}: {e}")

    logger.info(
        f"[图片清理] {sub_dir} 目录下共有 {result.scanned} 张图片,"
        f"其中时间早于 {base_time} 的有 {len(to_delete)} 张,实际删除了 {result.deleted} 张"
    )
    return result


def clean_logs_in_dir(debug_folder: Path, sub_dir: str, base_time: datetime) -> CleanupResult:
    """清理指定子目录下日期早于 base_time 的 YYYY-MM-DD.log 文件。"""
    result = CleanupResult()
    target_dir = debug_folder / sub_dir
    if not target_dir.exists():
        logger.info(f"[日志清理] 目录不存在,跳过: {target_dir}")
        return result

    log_files = list(target_dir.glob("*.log"))
    result.scanned = len(log_files)
    if result.scanned == 0:
        logger.info(f"[日志清理] {sub_dir} 目录下无日志文件,跳过")
        return result
    ...
    for log_path in log_files:
        ...
        if log_date is None:
            result.skipped += 1
            logger.warning(f"[日志清理] 无法解析日志日期,跳过: {log_path.name}")
            continue
    ...
    for log_date, log_path in to_delete:
        try:
            log_path.unlink()
            result.deleted += 1
            logger.info(f"[日志清理] 删除日志: {log_path.name} (日期 {log_date})")
        except Exception as e:
            result.errors += 1
            logger.error(f"[日志清理] 删除失败 {log_path.name}: {e}")

    logger.info(
        f"[日志清理] {sub_dir} 目录下共有 {result.scanned} 个日志文件,"
